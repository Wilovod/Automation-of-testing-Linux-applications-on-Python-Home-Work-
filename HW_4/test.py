...
class TestPositive:
    def test_step1(self, make_folders, clear_folders, make_files):
        ...
        res1 = ssh_checkout(data["ip"], data["user"], data["passwd"],
                            ...
                            "Everything is Ok")
        res2 = ssh_checkout(data["ip"], data["user"], data["passwd"], "ls -t{}".format(data["folder_out"]),
                            "arx.{}".format(data["type"]))
        assert res1 and res2, "test1 FAIL"

    def test_step2(self, clear_folders, make_files):
        # test2
        res = [ssh_checkout(data["ip"], data["user"], data["passwd"],
                            "cd {}; 7z a {}/arx -t{}".format(data["folder_in"], data["folder_out"], data["type"]),
                            "Everything is Ok"), ssh_checkout(data["ip"], data["user"], data["passwd"],
                                                              "cd {}; 7z e arx.{} -t{} -o{} -y".format(
                                                                  data["folder_out"], data["type"],
                                                                  data["folder_ext"]), "Everything is Ok")]
        for item in make_files:
            res.append(
                ssh_checkout(data["ip"], data["user"], data["passwd"], "ls -t{}".format(data["folder_ext"]), item))
        assert all(res)

    def test_step3(self):
        # test3
        assert ssh_checkout(data["ip"], data["user"], data["passwd"],
                            "cd {}; 7z t arx -t{}".format(data["folder_out"], data["type"]),
                            "Everything is Ok"), "test3 FAIL"

    def test_step4(self):
        # test4
        assert ssh_checkout(data["ip"], data["user"], data["passwd"],
                            "cd {}; 7z u arx -t{}".format(data["folder_in"], data["type"]),
    ...
    def test_step5(self, clear_folders, make_files):
        ...
        for i in make_files:
            res.append(ssh_checkout(data["ip"], data["user"], data["passwd"],
                                    "cd {}; 7z l arx -t{}".format(data["folder_out"], data["type"]), i))
    ...
    def test_step7(self):
        # test7
        assert ssh_checkout(data["ip"], data["user"], data["passwd"],
                            "cd {}; 7z d arx -t{}".format(data["folder_out"], data["type"]),
    ...
    def test_step8(self, clear_folders, make_files):
        ...
        for i in make_files:
            res.append(ssh_checkout(data["ip"], data["user"], data["passwd"],
                                    "cd {}; 7z h -t{} -t{}".format(data["folder_in"], i, data["type"]),
                                    "Everything is Ok"))
            hash = ssh_getout(data["ip"], data["user"], data["passwd"],
                              "cd {}; crc32 {}".format(data["folder_in"], i)).upper()
            res.append(ssh_checkout(data["ip"], data["user"], data["passwd"],
                                    "cd {}; 7z h -t{} -t{}".format(data["folder_in"], i, data["type"]), hash))
